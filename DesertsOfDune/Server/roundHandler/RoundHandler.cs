            ...
            else
            {
                ...
                if (IsOverlengthMechanismActive)
                {
                    ...
                    if (finishedGame)
                    {
                        ...
                        var stats = Party.GetInstance().GetPlayerByClientID(winnerID).statistics;
                        Party.GetInstance().messageController.DoGameEndMessage(winnerID, loserID, stats); //TODO: here just the stats of the winner are sent
            ...
            if (Party.GetInstance().AreTwoPlayersRegistred())
            {
                foreach (var player in Party.GetInstance().GetActivePlayers())
                {
                    if (player.UsedGreatHouse.Characters.Count == 0) //TODO: check, if in Characters.count are also defeated characters which are not cloned again yet
                    {
                        ...
                        var stats = Party.GetInstance().GetPlayerByClientID(winnerID).statistics;
                        Party.GetInstance().messageController.DoGameEndMessage(winnerID, loserID, stats); //TODO: get stats for both players
                        partyFinished = true;
                        return true;
                    }
                }
